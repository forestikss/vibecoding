...
public class DebugOverlay {
    ...
    private void drawRight(MatrixStack stack, List<String> strings) {
        for (int i = 0; i < strings.size(); i++) {
            ...
            stack.translate(mc.getMainWindow().getScaledWidth() - width - LEFT_PADDING, 25F + i * (Minecraft.getInstance().fontRenderer.lineHeight + 1F), 0F);
            AbstractGui.fill(stack, -1, -1, width, Minecraft.getInstance().fontRenderer.lineHeight, 0x90505050);
            Minecraft.getInstance().fontRenderer.drawShadow(stack, text, 0, 0, 0xFFFFFF);
