...
public class AudioDeviceEntry extends ListScreenEntryBase<AudioDeviceEntry> {
    ...
    protected static final ResourceLocation SELECTED = new ResourceLocation("minecraft", "textures/icons/device_selected.png");
    ...
    public void render(MatrixStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovered, float delta) {
        ...
        if (selected) {
            Minecraft.getInstance().getTextureManager().bindTexture(SELECTED);
            AbstractGui.blit(poseStack, left + PADDING, top + height / 2 - 8, 16, 16, 16, 16, 16, 16);
        }

        float deviceWidth = Minecraft.getInstance().fontRenderer.getStringWidth(visibleDeviceName);
        float space = width - PADDING - 16 - PADDING - PADDING;
        float scale = Math.min(space / deviceWidth, 1F);

        poseStack.push();
        poseStack.translate(left + PADDING + 16 + PADDING, top + height / 2 - (Minecraft.getInstance().fontRenderer.lineHeight * scale) / 2, 0D);
        poseStack.scale(scale, scale, 1F);

        Minecraft.getInstance().fontRenderer.drawShadow(poseStack, visibleDeviceName, 0, 0, DEVICE_NAME_COLOR);
        poseStack.pop();
    }
