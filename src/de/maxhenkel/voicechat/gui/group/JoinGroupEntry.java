...
public class JoinGroupEntry extends ListScreenEntryBase<JoinGroupEntry> {
    ...
    protected static final ResourceLocation LOCK = new ResourceLocation("minecraft", "textures/icons/lock.png");
    ...
    public void render(MatrixStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovered, float delta) {
        ...
        Minecraft.getInstance().fontRenderer.drawShadow(poseStack, groupName, left + PADDING + (hasPassword ? 16 + PADDING : 0), top + height / 2 - Minecraft.getInstance().fontRenderer.lineHeight / 2, PLAYER_NAME_COLOR);

        int textWidth = Minecraft.getInstance().fontRenderer.getStringWidth(groupName) + (hasPassword ? 16 + PADDING : 0);
        ...
        if (group.getMembers().isEmpty()) {
            ...
            for (int i = 0; i < group.getMembers().size(); i++) {
                if (i >= maxMembers) {
                    tooltip.add(new TranslationTextComponent("message.voicechat.more_members", group.getMembers().size() - maxMembers).mergeStyle(TextFormatting.GRAY).getVisualOrderText());
                    break;
                }
                PlayerState state = group.getMembers().get(i);
                tooltip.add(new StringTextComponent("  " + state.getName()).mergeStyle(TextFormatting.GRAY).getVisualOrderText());
            }
        }
