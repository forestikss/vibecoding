...
public final class KeyBindingRegistryImpl {
	...
	private static Map<String, Integer> getCategoryMap() {
		return KeyBinding.CATEGORY_ORDER;
	}
	...
	public static KeyBinding registerKeyBinding(KeyBinding binding) {
		for (KeyBinding existingKeyBindings : MODDED_KEY_BINDINGS) {
			if (existingKeyBindings == binding) {
				throw new IllegalArgumentException("Attempted to register a key binding twice: " + binding.getKeyDescription());
			} else if (existingKeyBindings.getKeyDescription().equals(binding.getKeyDescription())) {
				throw new IllegalArgumentException("Attempted to register two key bindings with equal ID: " + binding.getKeyDescription() + "!");
			}
		}

		// This will do nothing if the category already exists.
		addCategory(binding.getKeyCategory());
