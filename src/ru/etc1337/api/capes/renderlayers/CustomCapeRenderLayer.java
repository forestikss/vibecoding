package ru.etc1337.api.capes.renderlayers;
...
import ru.etc1337.api.capes.*;
import ru.etc1337.api.capes.math.Vector3;
import ru.etc1337.api.capes.sim.StickSimulation;
...
public class CustomCapeRenderLayer extends LayerRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> {
    ...
    private void modifyPoseStackSimulation(final MatrixStack poseStack, final AbstractClientPlayerEntity abstractClientPlayer, final float delta, final int part) {
        ...
        boolean isPlayer = abstractClientPlayer == Minecraft.getInstance().player;
        boolean isFriend = Client.getInstance().getFriendManager().isFriend(abstractClientPlayer);
        boolean flag = customModels.isEnabled() && (isPlayer || (customModels.friends.isEnabled() && isFriend));
        if (flag && customModels.mode.is("Rabbit")) {
            plus = 0.5F;
            zPlus = 0.225F;
        } else {
            plus = 0.0F;
        ...
        poseStack.translate(0.0, plus, zPlus);
    ...
    private void modifyPoseStackVanilla(final MatrixStack poseStack, final AbstractClientPlayerEntity abstractClientPlayer, final float h, final int part) {
        ...
        poseStack.translate(0.0D, plus, zPlus);
        ...
        if (abstractClientPlayer.isCrouching()) {
            height += 25.0f;
            poseStack.translate(0.0, 0.15000000596046448 + plus, zPlus);
        }
        final float naturalWindSwing = this.getNatrualWindSwing(part, abstractClientPlayer.canSwim());
