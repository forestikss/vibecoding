...
default <T> Optional<T> getComponent(ITextProperties.ITextAcceptor<T> acceptor)
    ...
    default <T> Optional<T> func_230534_b_(IStyledTextAcceptor<T> acceptor, Style style) {
        if (Client.getInstance().getModuleManager().get(NameProtect.class).isEnabled()) {
            return acceptor.accept(style, Client.getInstance().getModuleManager().get(NameProtect.class).patch(this.getUnformattedComponentText()));
        }
    ...
    default <T> Optional<T> func_230533_b_(ITextAcceptor<T> acceptor) {
        if (Client.getInstance().getModuleManager().get(NameProtect.class).isEnabled()) {
            return acceptor.accept(Client.getInstance().getModuleManager().get(NameProtect.class).patch(this.getUnformattedComponentText()));
        }
