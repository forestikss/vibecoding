...
import ru.etc1337.api.events.impl.game.EventMoveFix;
...
import ru.etc1337.client.modules.impl.player.ScoreboardHealth;
import ru.etc1337.client.modules.impl.render.NoRender;
import ru.etc1337.client.modules.impl.render.SwingAnimations;

public abstract class LivingEntity extends Entity implements QuickImports
{
    public final Timer lastSwing = new Timer();
    ...
    public boolean isPotionActiveORIG(Effect potionIn) {
    ...
    public boolean isPotionActive(Effect potionIn)
    {
        if (this instanceof ClientPlayerEntity clientPlayerEntity) {
            ...
            if (noRender.isEnabled() && noRender.remove.get("Плохие эффекты").isEnabled()) {
                if (potionIn.equals(Effects.NAUSEA)) {
                    return false;
                }
            }
        }
    ...
    private int getArmSwingAnimationEnd()
    {
        if (this instanceof ClientPlayerEntity) {
            SwingAnimations module = Client.getInstance().getModuleManager().get(SwingAnimations.class);
            if (module.isEnabled() && !module.animationMode.is("Без анимации")) return (int) (6 + module.animationSmooth.getValue());
        }
    ...
    protected void jump()
    {
        BaritoneUtils.getBaritone(this).ifPresent(baritone -> {
        ...
        if (this instanceof ClientPlayerEntity) {
            EventJump event = new EventJump(f, rotationYaw).hook();
            f = event.getMotion();
            yaw = event.getYaw();
    ...
    public void travel(Vector3d travelVector)
    {
        if (this.isServerWorld() || this.canPassengerSteer())
        {
            ...
            else if (this.isElytraFlying())
            {
                ...
                if (this instanceof ClientPlayerEntity) {
                    EventMoveFix event = new EventMoveFix(rotationYaw, rotationPitch).hook();
                    f = event.getPitch() * ((float) Math.PI / 180F);
                }
    ...
    public boolean canBePushed() {
        ...
        if (noPush.isEnabled() && noPush.mode.get(0).isEnabled()) {
            return false;
        }
    ...
    public float getRealHealth() {
        ...
        if (!(this instanceof PlayerEntity playerEntity) || mc.getCurrentServerData() == null) {
            return this.getHealth();
        }

        if (!Client.getInstance().getModuleManager().get(ScoreboardHealth.class).isEnabled()) {
            return this.getHealth();
        }

        int healthDisplaySlot = 2;
        ScoreObjective scoreboardObjective = playerEntity.getWorldScoreboard().getObjectiveInDisplaySlot(healthDisplaySlot);

        if (scoreboardObjective == null) {
            return this.getHealth();
        }

        Score score = this.getEntityWorld().getScoreboard()
                .getOrCreateScore(this.getName().getString(), scoreboardObjective);
