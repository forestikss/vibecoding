...
import ru.etc1337.client.modules.impl.misc.TpsSync;
...
public abstract class PlayerEntity extends LivingEntity
{
    ...
    public void tick()
    {
        this.noClip = this.isSpectator() || Client.getInstance().getModuleManager().get(FreeCam.class).isEnabled();
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag5)
                    {
                        if (i > 0) {
                            ...
                            if (!new EventKeepSprint().hook().isCancelled()) {
    ...
    public void travel(Vector3d travelVector)
    {
        ...
        {
            Vector3d vector3d12 = this.getLookVec();
            double d3 = vector3d12.y;
    ...
    public IFormattableTextComponent addTellEvent(IFormattableTextComponent p_208016_1_)
    {
    ...
    {
        TpsSync tpsSync = Client.getInstance().getModuleManager().get(TpsSync.class);
        double diff = Math.abs(tpsSync.getTps() - 20.0D);
        double tps = tpsSync.mode.get("Удары").isEnabled() ? 20.0D + diff : 20.0D;
        return (float)(1.0D / this.getAttributeValue(Attributes.ATTACK_SPEED) * tps);
    }
