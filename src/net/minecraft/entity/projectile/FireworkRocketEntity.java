        ...
        if (this.isAttachedToEntity())
        {
            ...
            if (this.boostedEntity != null)
            {
                ...
                if (this.boostedEntity.isElytraFlying()) {
                    ...
                    if (boostedEntity instanceof ClientPlayerEntity) {
                        final EventMoveFix event = new EventMoveFix(boostedEntity.rotationYaw, boostedEntity.rotationPitch).hook();
                        vector3d = this.getVectorForRotation(event.getPitch(), event.getYaw());
                    }

                    Vector3d vector3d1 = this.boostedEntity.getMotion();
                    Vector3d multiplier = new Vector3d(1.5D, 1.5D, 1.5D);
                    if (boostedEntity instanceof ClientPlayerEntity) {
                        final EventFireworkMotion event = new EventFireworkMotion(boostedEntity, this,
                                new Vector3d(1.6D, 1.6D, 1.6D)).hook();
                        if (event.isCancelled()) {
                            multiplier = event.getVector3d();
                            if (multiplier.equals(Vector3d.ZERO)) return;
                        }
                    }


                    Vector3d motion = vector3d1.add(
                            vector3d.x * 0.1D + (vector3d.x * multiplier.x - vector3d1.x) * 0.5D,
                            vector3d.y * 0.1D + (vector3d.y * multiplier.y - vector3d1.y) * 0.5D,
                            vector3d.z * 0.1D + (vector3d.z * multiplier.z - vector3d1.z) * 0.5D
                    );

                    this.boostedEntity.setMotion(motion);
                    vec3 = this.boostedEntity.getHandHoldingItemAngle(Items.FIREWORK_ROCKET);
                } else {
                    vec3 = Vector3d.ZERO;
                }

                this.setPosition(this.boostedEntity.getPosX() + vec3.x, this.boostedEntity.getPosY() + vec3.y, this.boostedEntity.getPosZ() + vec3.z);
