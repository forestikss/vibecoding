...
import ru.etc1337.api.baritone.api.BaritoneAPI;
import ru.etc1337.api.baritone.api.BaritoneUtils;
import ru.etc1337.api.baritone.api.IBaritone;
import ru.etc1337.api.baritone.api.event.events.ChatEvent;
import ru.etc1337.api.baritone.api.event.events.PlayerUpdateEvent;
import ru.etc1337.api.baritone.api.event.events.type.EventState;
import ru.etc1337.api.baritone.common.behavior.LookBehavior;
import ru.etc1337.api.events.impl.game.*;
import ru.etc1337.client.modules.impl.movement.AutoSprint;
import ru.etc1337.client.modules.impl.movement.NoSlow;
import ru.etc1337.client.modules.impl.player.NoPush;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity
{
    ...
    {
        EventMotion event = new EventMotion(rotationYaw, rotationPitch, onGround).hook();
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            if (this.isPassenger())
            {
                ...
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(vector3d.x, -999.0D, vector3d.z, event.getYaw(), event.getPitch(), event.isGround()));
                flag1 = false;
            }
            else if (flag1 && flag2)
            {
                this.connection.sendPacket(new CPlayerPacket.PositionRotationPacket(this.getPosX(), this.getPosY(), this.getPosZ(), event.getYaw(), event.getPitch(), event.isGround()));
            }
            else if (flag1)
            {
                this.connection.sendPacket(new CPlayerPacket.PositionPacket(this.getPosX(), this.getPosY(), this.getPosZ(), event.isGround()));
            }
            else if (flag2)
            {
                this.connection.sendPacket(new CPlayerPacket.RotationPacket(event.getYaw(), event.getPitch(), event.isGround()));
            }
            else if (this.prevOnGround != event.isGround())
            {
                this.connection.sendPacket(new CPlayerPacket(event.isGround()));
            }
            ...
            this.prevOnGround = event.isGround();
    ...
    public void sendChatMessage(String message)
    {
        ...
        if (!new EventSendMessage(message).hook().isCancelled()) {
    ...
    public void swingArm(Hand hand)
    {
        ...
        if (!(swingAnimations.animationMode.is("Swingless") && swingAnimations.isEnabled())) {
            super.swingArm(hand);
        }
    ...
    {
        if (!new EventInventoryClose(mc.currentScreen, this.openContainer.windowId).hook().isCancelled()) {
    ...
    {
        NoPush noPush = Client.getInstance().getModuleManager().get(NoPush.class);
        if (noPush.isEnabled() && noPush.mode.get(1).isEnabled()) {
            return false;
        }
    ...
    public void livingTick()
    {
        ...
        {
            if (!new EventNoSlow().hook().isCancelled()) {
        ...
        NoSlow noSlow = Client.getInstance().getModuleManager().get(NoSlow.class);
        boolean isHandActive = this.isHandActive() && !(noSlow.isEnabled() && noSlow.sprintWhileUsing.isEnabled());
        ...
        boolean canSprint = autoSprint.isCanSprint();
        boolean keySprintDown = canSprint && (BaritoneUtils.isKeyDown(this.mc.gameSettings.keyBindSprint, this) || autoSprint.isEnabled());
        if (!canSprint) {
            this.setSprinting(false);
        }
        if ((this.onGround || this.canSwim()) && !flag1 && !flag2 && this.isUsingSwimmingAnimation() && !this.isSprinting() && flag4 && !isHandActive && !this.isPotionActive(Effects.BLINDNESS))
        {
            if (this.sprintToggleTimer <= 0 && !keySprintDown)
            {
        ...
        if (!this.isSprinting() && (!this.isInWater() || this.canSwim()) && this.isUsingSwimmingAnimation() && flag4 && !isHandActive && !this.isPotionActive(Effects.BLINDNESS) && keySprintDown)
        {
            this.setSprinting(true);
        }
