...
public class DownloadingPackFinder implements IPackFinder
{
   ...
   /*     this.lockDownload.lock();
    ...
    public CompletableFuture<Void> setServerPack1(File fileIn, IPackNameDecorator p_217816_2_)
    {
    ...
    public CompletableFuture<Void> setServerPack(File fileIn, IPackNameDecorator source) {
        PackMetadataSection packmetadatasection;
        try {
            FilePack filepackresources = new FilePack(fileIn);

            try {
                packmetadatasection = filepackresources.getMetadata(PackMetadataSection.SERIALIZER);
        ...
        ResourcePackInfo newServerPack = new ResourcePackInfo("server", true,
                () -> new FilePack(fileIn), new TranslationTextComponent("resourcePack.server.name"),
                packmetadatasection.getDescription(), PackCompatibility.getCompatibility(packmetadatasection.getPackFormat()), ResourcePackInfo.Priority.TOP, true, source);
