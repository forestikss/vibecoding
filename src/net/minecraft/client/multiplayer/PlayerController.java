...
import ru.etc1337.api.baritone.common.utils.accessor.IPlayerControllerMP;
...
import ru.etc1337.client.modules.impl.misc.NoInteract;
...
public class PlayerController implements IPlayerControllerMP
{
    ...
    {
        NoInteract noInteract = Client.getInstance().getModuleManager().get(NoInteract.class);
        this.syncCurrentPlayItem();
        ...
        else
        {
            ...
            else
            {
                ...
                boolean flag2 = noInteract.isEnabled() && noInteract.allBlocks.isEnabled() || noInteract.isEnabled() && noInteract.getBlocks().contains(p_217292_2_.getBlockState(p_217292_4_.getPos()).getBlockId()) || Client.getInstance().getModuleManager().get(KillAura.class).getTarget() != null;
                if (!flag2) {
                    if (!flag1) {
    ...
    {
        if (targetEntity instanceof LivingEntity living && new EventAttack(living).hook().isCancelled()) return;
