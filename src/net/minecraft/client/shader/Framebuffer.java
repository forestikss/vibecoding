...
public class Framebuffer {
    ...
    public Framebuffer(int p_i51175_1_, int p_i51175_2_, boolean p_i51175_3_, boolean isMac) {
    ...
    public Framebuffer(int p_i51175_1_, int p_i51175_2_, boolean useDepthTest) {
        RenderSystem.assertThread(RenderSystem::isOnRenderThreadOrInit);
        this.useDepth = useDepthTest;
        this.framebufferObject = -1;
        this.framebufferTexture = -1;
        this.depthBuffer = -1;
        this.framebufferColor = new float[4];
        ...
        this.framebufferColor[2] = 1.0F;
        this.framebufferColor[3] = 0.0F;
        this.resize(p_i51175_1_, p_i51175_2_, false);
    }
    ...
    public void framebufferRenderExtRaw(int width, int height, boolean p_227588_3_) {
        if (GLX.isUsingFBOs()) {
            RenderSystem.assertThread(RenderSystem::isOnRenderThread);
    ...
    public void framebufferRender(int width, int height, boolean p_227588_3_) {
        if (GLX.isUsingFBOs()) {
            RenderSystem.assertThread(RenderSystem::isOnRenderThread);
            GlStateManager.colorMask(true, true, true, false);
            GlStateManager.disableDepthTest();
            ...
            float f = (float) width;
            float f1 = (float) height;
            float f2 = (float) this.framebufferWidth / (float) this.framebufferTextureWidth / 2;
            float f3 = (float) this.framebufferHeight / (float) this.framebufferTextureHeight / 2;
            Tessellator tessellator = RenderSystem.renderThreadTesselator();
            BufferBuilder bufferbuilder = tessellator.getBuffer();
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
            bufferbuilder.pos(0.0D, f1, 0.0D).tex(0.0F, 0.0F).color(255, 255, 255, 255).endVertex();
