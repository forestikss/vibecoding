...
public class OptionSlider extends GameSettingsSlider implements IBidiTooltip, IOptionControl
{
    ...
    protected void applyValue()
    {
        if (!this.adjusting)
        {
            ...
            double d1 = this.option.denormalizeValue(this.value);

            if (d1 != d0)
            {
                this.option.set(this.settings, this.option.denormalizeValue(this.value));
    ...
    protected void updateMessage()
    {
        if (this.adjusting)
        {
            double d0 = this.option.denormalizeValue(this.value);
