...
import ru.etc1337.client.modules.impl.misc.ItemScroller;

public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T>, QuickImports
{
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        for (int i1 = 0; i1 < this.container.inventorySlots.size(); ++i1)
        {
            ...
            if (this.isSlotSelected(slot, (double)mouseX, (double)mouseY) && slot.isEnabled()) {
                ItemScroller itemScroller = Client.getInstance().getModuleManager().get(ItemScroller.class);
                if (itemScroller.isEnabled()) {
                    if (GLFW.glfwGetMouseButton(window.getHandle(), GLFW.GLFW_MOUSE_BUTTON_LEFT) == GLFW.GLFW_PRESS && GLFW.glfwGetKey(window.getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT) == GLFW.GLFW_PRESS) {
                        if (timer.finished((long) itemScroller.delay.getValue()) && slot.getHasStack()) {
    ...
    {
        if (new EventSlotClick(slotIn, type).hook().isCancelled()) {
            return;
        }
        if (slotIn != null)
        {
