...
import ru.etc1337.api.baritone.api.event.events.ChunkEvent;
import ru.etc1337.api.baritone.api.event.events.type.EventState;
import ru.etc1337.api.baritone.common.Baritone;
import ru.etc1337.api.baritone.common.cache.CachedChunk;
import ru.etc1337.api.events.impl.game.EventLoadWorld;
...
public class ClientPlayNetHandler implements IClientPlayNetHandler
{
    ...
    public void onDisconnect(ITextComponent reason)
    {
        ...
        {
            this.client.displayGuiScreen(new DisconnectedScreen(new MultiplayerScreen(new MainMenu()), field_243491_b, reason));
        }
    }
        ...
        else
        {
            ...
            for (int i : packetIn.getPassengerIds())
            {
                ...
                if (entity1 != null)
                {
                    ...
                    {
                        this.client.ingameGUI.setOverlayMessage(new TranslationTextComponent("mount.onboard", this.client.gameSettings.keyBindSneak.getTranslatedKeyMessage()), false);
                    }
                }
            }
        }
    ...
    public void handleChangeGameState(SChangeGameStatePacket packetIn)
    {
        ...
        else if (schangegamestatepacket$state == SChangeGameStatePacket.field_241769_f_)
        {
            ...
            {
                this.client.ingameGUI.getChatGUI().printChatMessage(new TranslationTextComponent("demo.help.movement", gamesettings.keyBindForward.getTranslatedKeyMessage(), gamesettings.keyBindLeft.getTranslatedKeyMessage(), gamesettings.keyBindBack.getTranslatedKeyMessage(), gamesettings.keyBindRight.getTranslatedKeyMessage()));
            }
            else if (f == 102.0F)
            {
                this.client.ingameGUI.getChatGUI().printChatMessage(new TranslationTextComponent("demo.help.jump", gamesettings.keyBindJump.getTranslatedKeyMessage()));
            }
            else if (f == 103.0F)
            {
                this.client.ingameGUI.getChatGUI().printChatMessage(new TranslationTextComponent("demo.help.inventory", gamesettings.keyBindInventory.getTranslatedKeyMessage()));
            }
            else if (f == 104.0F)
            {
                this.client.ingameGUI.getChatGUI().printChatMessage(new TranslationTextComponent("demo.day.6", gamesettings.keyBindScreenshot.getTranslatedKeyMessage()));
            }
        }
    ...
    public void handleCustomPayload(SCustomPayloadPlayPacket packetIn)
    {
        ...
        try
        {
            ...
            else if (SCustomPayloadPlayPacket.field_229729_o_.equals(resourcelocation))
            {
                ...
                if (!resourcelocation.getNamespace().contains("voicechat")) {
