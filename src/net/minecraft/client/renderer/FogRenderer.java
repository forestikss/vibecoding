...
public class FogRenderer
{
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier)
    {
        ...
        boolean ignore = noRender.isEnabled() && noRender.remove.get("Плохие эффекты").isEnabled();
        ...
        else
        {
            ...
            EventWorldColor eventWorldColor = new EventWorldColor((float) vector3d4.getX(), (float) vector3d4.getY(), (float) vector3d4.getZ());
            eventWorldColor.hook();
            float eventRed = eventWorldColor.getRed();
            float eventGreen = eventWorldColor.getGreen();
            float eventBlue = eventWorldColor.getBlue();
            red = eventRed;
            green = eventGreen;
            blue = eventBlue;
        ...
        if (!ignore && (activeRenderInfoIn.getRenderViewEntity() instanceof LivingEntity && ((LivingEntity)activeRenderInfoIn.getRenderViewEntity()).isPotionActive(Effects.BLINDNESS)))
        {
    ...
    public static void setupFog(ActiveRenderInfo p_setupFog_0_, FogRenderer.FogType p_setupFog_1_, float p_setupFog_2_, boolean p_setupFog_3_, float p_setupFog_4_)
    {
        NoRender noRender = Client.getInstance().getModuleManager().get(NoRender.class);
        boolean ignore = noRender.isEnabled() && noRender.remove.get("Плохие эффекты").isEnabled();
        CustomWorld customFog = Client.getInstance().getModuleManager().get(CustomWorld.class);
        ...
        else
        {
            ...
            else if (!ignore && entity instanceof LivingEntity && ((LivingEntity)entity).isPotionActive(Effects.BLINDNESS))
            {
