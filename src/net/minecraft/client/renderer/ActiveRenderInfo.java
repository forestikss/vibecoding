...
import ru.etc1337.api.events.impl.game.EventRotation;
...
public class ActiveRenderInfo
{
    ...
    @Getter private boolean thirdPersonReverse;
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks)
    {
        ...
        EventRotation event = new EventRotation(renderViewEntity.getYaw(partialTicks), renderViewEntity.getPitch(partialTicks), partialTicks);
    ...
    private double calcCameraDistance(double startingDistance)
    {
        ...
        if (!(noRender.isEnabled() && noRender.remove.get("Ограничение просмотра").isEnabled())) {
            for (int i = 0; i < 8; ++i) {
