...
import ru.etc1337.client.modules.impl.render.SwingAnimations;
import ru.etc1337.client.modules.impl.render.ViewModel;
...
public class FirstPersonRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer.Impl bufferIn, ClientPlayerEntity playerEntityIn, int combinedLightIn)
    {
        ...
        if (!(noRender.isEnabled() && noRender.remove.get("Интерполяцию руки").isEnabled())) {
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn))
        {
            ...
            matrixStackIn.push();
            ViewModel viewModel = Client.getInstance().getModuleManager().get(ViewModel.class);
            if (viewModel.isEnabled()) {
                if (handside == HandSide.RIGHT) {
                    matrixStackIn.translate(viewModel.rightX.getValue(), viewModel.rightY.getValue(), viewModel.rightZ.getValue());
                } else if (handside == HandSide.LEFT) {
                    matrixStackIn.translate(viewModel.leftX.getValue(), viewModel.leftY.getValue(), viewModel.leftZ.getValue());
                }
            }
            if (stack.isEmpty())
            {
            ...
            else
            {
                ...
                {
                    SwingAnimations swingAnimations = Client.getInstance().getModuleManager().get(SwingAnimations.class);
                    ...
                    int l = flag3 ? 1 : -1;
                    if (swingAnimations.isEnabled() && !swingAnimations.animationMode.is("Swingless") && handside == HandSide.RIGHT) {
                        switch (swingAnimations.animationMode.getCurrentMode()) {
                            case "Smooth Translate": {
                                this.smoothTranslate(swingProgress, swingAnimations, matrixStackIn);
                                break;
                            }
                            case "Smooth Old": {
                                this.smoothOld(partialTicks, swingAnimations, matrixStackIn);
                                break;
                            }
                            case "Smooth": {
                                this.smooth(swingProgress, handside, swingAnimations, l, matrixStackIn);
                                break;
                            }
                            case "Jump Translate": {
                                this.jumpTranslate(swingAnimations, f10, matrixStackIn);
                                break;
                            }
                            case "Default": {
                                this.defaultAnim(swingProgress, swingAnimations, matrixStackIn);
                                break;
                            }
                            case "Fap": {
                                this.fap(swingAnimations, f10, matrixStackIn);
                                break;
                            }
                            case "Snap" : {
                                this.snap(matrixStackIn, swingProgress, l, swingAnimations);
                                break;
                            }
                            case "Smooth Back" : {
                                this.smoothBack(matrixStackIn, swingProgress, l, swingAnimations);
                                break;
                            }
                            case "Big": {
                                this.big(swingAnimations, partialTicks, matrixStackIn);
                            }
                        }
                    } else {
    ...
    private void smoothOld(float f2, SwingAnimations swingAnimations, MatrixStack stack) {
        stack.translate(0.4f, 0.0f, -0.8f);
        stack.translate(0.56f, -0.44f, -0.71999997f);
        stack.rotate(Vector3f.YP.rotationDegrees(72.0f));

        float f3 = MathHelper.sin(MathHelper.sqrt(mc.player.getSwingProgress(f2)) * (float)Math.PI);

        stack.rotate(Vector3f.ZP.rotationDegrees(f3 * -10.0f));
        stack.rotate(Vector3f.XP.rotationDegrees(f3 * (float)(-swingAnimations.animationStrength.getValue())));
        stack.rotate(Vector3f.XP.rotationDegrees(-20.0f));

        stack.translate(-0.5f, 0.08f, 0.0f);

        stack.rotate(Vector3f.YP.rotationDegrees(20.0f));
        stack.rotate(Vector3f.XP.rotationDegrees(-80.0f));
        stack.rotate(Vector3f.YP.rotationDegrees(20.0f));
    }

    private void snap(MatrixStack matrixStack, float swingProgress, float f3, SwingAnimations swingAnimations) {
        matrixStack.translate(f3 * 0.56f, -0.52f, -0.72f);
        matrixStack.multiply(Vector3f.XP.rotationDegrees(swingProgress * -360 * swingAnimations.animationSmooth.getValue()));
    }

    private void smoothBack(MatrixStack matrixStack, float swingProgress, float f3, SwingAnimations swingAnimations) {
        matrixStack.translate(f3 * 0.56f, -0.52f, -0.72f);

        matrixStack.rotate(Vector3f.YP.rotationDegrees((float)swingProgress *
                (45.0f + MathHelper.sin(f3 * f3 * (float)Math.PI) / 4.0f * -20.0f)));
        matrixStack.rotate(Vector3f.ZP.rotationDegrees((float)swingProgress * MathHelper.sin(swingProgress * (float)Math.PI) * -20.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(MathHelper.sin(swingProgress * (float)Math.PI) * (float)(-swingAnimations.animationStrength.getValue())));

        matrixStack.rotate(Vector3f.YP.rotationDegrees((float)swingProgress * -45.0f));
    }

    private void fap(SwingAnimations swingAnimations, float f2, MatrixStack matrixStack) {
        matrixStack.translate(1.2f, -0.1f, -0.71999997f);
        matrixStack.translate(0.0f, -0.0f, 0.0f);
        matrixStack.rotate(Vector3f.YP.rotationDegrees(45.0f));

        float f3 = MathHelper.sin(0.0f);
        float f4 = MathHelper.sin(MathHelper.sqrt(0.0f) * (float)Math.PI);

        matrixStack.rotate(Vector3f.YP.rotationDegrees(f3 * -20.0f));
        matrixStack.rotate(Vector3f.ZP.rotationDegrees(f4 * -20.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(f4 * -80.0f));

        matrixStack.translate(-0.5f, 0.2f, 0.0f);
        ...
        int n2 = (int)Math.min(255L, (System.currentTimeMillis() % 255L > 127L ? Math.abs(Math.abs(System.currentTimeMillis()) % 255L - 255L) : System.currentTimeMillis() % 255L) * 2L);
        float f5 = (double)f2 > 0.5 ? 1.0f - f2 : f2;

        matrixStack.translate(0.3f, -0.0f, 0.4f);
        matrixStack.rotate(Vector3f.ZP.rotationDegrees(0.0f));
        matrixStack.translate(0.0f, 0.5f, 0.0f);
        matrixStack.rotate(Vector3f.XP.rotationDegrees(90.0f));

        matrixStack.translate(0.6f, 0.5f, 0.0f);
        matrixStack.rotate(Vector3f.XP.rotationDegrees(-90.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(-10.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(-f5 * 10.0f));
        matrixStack.rotate(Vector3f.YP.rotationDegrees(10.0f));

        matrixStack.translate(0.0F, 0.0F, -0.5F);

        // Для вращения во время анимации используем "matrixStack.rotate", учитывая состояние игрока
        matrixStack.rotate(Vector3f.ZP.rotationDegrees(mc.player.isSwingInProgress ? (float)(-n2) / (110.0f - swingAnimations.animationStrength.getValue()) : 1.0f));

        matrixStack.translate(0.0F, 0.0F, 0.5F);
    }

    private void defaultAnim(float f2, SwingAnimations swingAnimations, MatrixStack matrixStack) {
        float f3 = -swingAnimations.animationStrength.getValue() * (float)Math.sin((double)f2 * 1.5707963267948966 * 2.0) * swingAnimations.animationStrength.getValue() / 100.0f;
    ...
    private void big(SwingAnimations swingAnimations, float f2, MatrixStack matrixStack) {
        float f3 = mc.player.getSwingProgress(f2);

        matrixStack.translate(0.5f, -0.3f, -0.65f);
        matrixStack.translate(0.0f, 0.0f, 0.0f);
        matrixStack.rotate(Vector3f.YP.rotationDegrees(45.0f));

        float f4 = MathHelper.sin(f3 * f3 * (float)Math.PI);
        float f5 = MathHelper.sin(MathHelper.sqrt(f3) * (float)Math.PI);

        matrixStack.rotate(Vector3f.YP.rotationDegrees(f4 * -20.0f));
        matrixStack.rotate(Vector3f.ZP.rotationDegrees(f5 * -20.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(f5 * (float)(-swingAnimations.animationStrength.getValue())));
    ...
    private void smooth(float f2, HandSide hF, SwingAnimations swingAnimations, float f3, MatrixStack matrixStack) {
        matrixStack.translate(f3 * 0.56f, -0.52f, -0.72f);

        int n2 = hF == HandSide.RIGHT ? 1 : -1;
        float f4 = MathHelper.sqrt(f2);

        // Применение вращения с учетом стороны руки и анимации
        matrixStack.rotate(Vector3f.YP.rotationDegrees((float)n2 * (45.0f + MathHelper.sin(f2 * f2 * (float)Math.PI) / 4.0f * -20.0f)));
        matrixStack.rotate(Vector3f.ZP.rotationDegrees((float)n2 * MathHelper.sin(f4 * (float)Math.PI) * -20.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(MathHelper.sin(f4 * (float)Math.PI) * (float)(-swingAnimations.animationStrength.getValue())));

        matrixStack.rotate(Vector3f.YP.rotationDegrees((float)n2 * -45.0f));
    }


    private void jumpTranslate(SwingAnimations swingAnimations, float f2, MatrixStack matrixStack) {
        float f3 = (float)((double)MathHelper.sqrt(f2) * Math.PI);

        matrixStack.translate(0.4f, -0.15f, -0.5f);
        matrixStack.translate(0.0f, f2 * -swingAnimations.animationStrength.getValue() / 100.0f, 0.0f);
        matrixStack.rotate(Vector3f.YP.rotationDegrees(45.0f));
        matrixStack.rotate(Vector3f.YP.rotationDegrees(MathHelper.sin((float)((double)(f2 * f2) * Math.PI)) * -34.0f));
        matrixStack.rotate(Vector3f.ZP.rotationDegrees(MathHelper.sin(f3) * -20.7f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(MathHelper.sin(f3) * -68.6f));
    ...
    private void smoothTranslate(float f2, SwingAnimations swingAnimations, MatrixStack matrixStack) {
        matrixStack.translate(0.6f, 0.0f, -0.8f);
        matrixStack.translate(0.56f, -1.0f, -0.82f);
        matrixStack.rotate(Vector3f.YP.rotationDegrees(90.0f));
        matrixStack.rotate(Vector3f.ZP.rotationDegrees(-60.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees((float)(-swingAnimations.rotationAngle.getValue()) - 40.0f * (float)Math.sin((double)f2 * 1.5707963267948966 * 2.0) * swingAnimations.animationStrength.getValue() / 50.0f));
        matrixStack.rotate(Vector3f.XP.rotationDegrees(-20.0f));
        matrixStack.translate(-0.5f, 0.08f, 0.0f);
    }
