...
import ru.etc1337.api.events.impl.game.EventTrace;
...
import ru.etc1337.client.modules.impl.misc.Beautifully;
import ru.etc1337.client.modules.impl.render.AspectRatio;
import ru.etc1337.client.modules.impl.render.CustomWorld;
import ru.etc1337.client.modules.impl.render.NoRender;
import ru.etc1337.client.modules.impl.render.TotemAnimation;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            EventTrace event = new EventTrace(entity.rotationYaw, entity.rotationPitch).hook();
            if (event.isCancelled()) {
                vector3d1 = entity.getVectorForRotation(event.getPitch(), event.getYaw());
            }
...
*/
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            Beautifully beautifully = Client.getInstance().getModuleManager().get(Beautifully.class);
            boolean isBeat = beautifully.isEnabled() && beautifully.mode.get("Улучшенный зум").isEnabled();
            boolean flag = false;
    ...
    private void hurtCameraEffect(MatrixStack matrixStackIn, float partialTicks) {
        ...
        if (noRender.isEnabled() && noRender.remove.get("Тряска при ударе").isEnabled()) return;
...
/*        matrixstack.getLast().getMatrix().mul(Matrix4f.perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting), (float)this.mc.getMainWindow().getFramebufferWidth() / (float)this.mc.getMainWindow().getFramebufferHeight(), 0.05F, this.clipDistance));
        ...
        matrixstack.getLast().getMatrix()
                .mul(Matrix4f.perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting),
                        (float) this.mc.getMainWindow().getFramebufferWidth() / (float) this.mc.getMainWindow().getFramebufferHeight() *
                                (aspectRatio != null && aspectRatio.isEnabled() ? aspectRatio.getAspect().getValue() : 1),
                        0.05F, clipDistance));
        return matrixstack.getLast().getMatrix();
    }
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
        ...
        float dist = 1F;
        CustomWorld customWorld = Client.getInstance().getModuleManager().get(CustomWorld.class);
        if (customWorld.isEnabled()) {
            dist = customWorld.getDistance().getValue();
        }
        ...
        new EventRender3D(matrixStackIn, partialTicks, activeRender).hookRender();
    ...
    public void displayItemActivation(ItemStack stack) {
        TotemAnimation totemAnimation = Client.getInstance().getModuleManager().get(TotemAnimation.class);
        if (totemAnimation.isEnabled()) {
            totemAnimation.displayItemActivation(stack);
        } else {
            NoRender noRender = Client.getInstance().getModuleManager().get(NoRender.class);
            if (noRender.isEnabled() && noRender.remove.get("Анимацию тотема").isEnabled()) {
                return;
            }
    ...
    private void renderItemActivation(int widthsp, int heightScaled, float partialTicks) {
        ...
        TotemAnimation totemAnimation = Client.getInstance().getModuleManager().get(TotemAnimation.class);
        if (totemAnimation.isEnabled()) {
            totemAnimation.renderItemActivation(matrixstack, partialTicks);
        } else if (this.itemActivationItem != null && this.itemActivationTicks > 0) {
