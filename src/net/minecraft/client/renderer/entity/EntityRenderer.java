...
import ru.etc1337.client.modules.impl.render.EntityESP;
...
public abstract class EntityRenderer<T extends Entity> implements net.optifine.entity.model.IEntityRenderer
{
    ...
    {
        EntityESP entityESP = Client.getInstance().getModuleManager().get(EntityESP.class);
        if (entityESP.isEnabled() && entityESP.getChecks().get("Никнейм").isEnabled() && entityIn instanceof PlayerEntity) {
            return;
        }
