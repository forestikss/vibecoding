...
import de.maxhenkel.voicechat.VoiceChatInstance;
...
import ru.etc1337.api.baritone.api.event.events.TickEvent;
import ru.etc1337.api.baritone.api.event.events.WorldEvent;
import ru.etc1337.api.baritone.api.event.events.type.EventState;
import ru.etc1337.api.baritone.api.utils.RayTraceUtils;
...
import ru.etc1337.api.viamcp.fixes.AttackOrder;
import ru.etc1337.client.modules.impl.combat.AutoGApple;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener
{
    ...
    public Minecraft(GameConfiguration gameConfig)
    {
        ...
        this.mainWindow.setWindowTitle("Stradix » 1.16.5");
        this.setGameFocused(true);

        try
        {
            InputStream inputstream = this.getPackFinder().getVanillaPack().getResourceStream(ResourcePackType.CLIENT_RESOURCES, new ResourceLocation("dreamcore/images/logo/avatar.png"));
            InputStream inputstream1 = this.getPackFinder().getVanillaPack().getResourceStream(ResourcePackType.CLIENT_RESOURCES, new ResourceLocation("dreamcore/images/logo/avatar.png"));
        ...
        this.skinManager = new SkinManager(this.textureManager, new File(Constants.CACHE_DIRECTORY, "skins"), this.sessionService);
        ...
        if (s != null)
        {
            this.displayGuiScreen(new ConnectingScreen(new MainMenu(), this, s, i));
        }
        else
        {
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn)
    {
        ...
        if (guiScreenIn != null)
        {
            ...
            //sosal.saysosal(guiScreenIn.getNarrationMessage());
        }
        else
        {
                ...
                for (Hand hand : Hand.values())
                {
                    ...
                    if (this.objectMouseOver != null)
                    {
                        switch (this.objectMouseOver.getType())
                        {
                            ...
                            case BLOCK:
                                RayTraceResult rayTraceResult = RayTraceUtils.rayTrace(playerController.getBlockReachDistance(),
                                        this.player.rotationYaw,
                                        this.player.rotationPitch,
                                        player, RayTraceContext.BlockMode.RAYTRACE_SHAPE);
                           /*     if (rayTraceResult.getType() == RayTraceResult.Type.BLOCK && rayTraceResult instanceof BlockRayTraceResult traceResult && Client.inst().featureManager().openWalls.enabled()) {
                                    ActionResultType actionresulttype1 = playerController.rightClickBlock(player, world, hand, traceResult);
        ...
        if (this.world != null)
        {
            if (!this.isGamePaused)
            {
                if (!this.gameSettings.field_244601_E && this.func_244600_aM())
                {
                    ...
                    ITextComponent itextcomponent1 = new TranslationTextComponent("tutorial.socialInteractions.description", createKeybindComponent("socialInteractions"));
    ...
    private void processKeyBinds()
    {
        ...
        while (this.gameSettings.field_244602_au.isPressed())
        {
            if (!this.func_244600_aM())
            {
                ...
                //sosal.saysosal(field_244596_I.getString());
            }
            else
            {
        ...
        AutoGApple autoGApple = Client.getInstance().getModuleManager().get(AutoGApple.class);
        if (currentScreen != null && !(autoGApple.isEnabled() && autoGApple.canEat())) {
            return;
        }
